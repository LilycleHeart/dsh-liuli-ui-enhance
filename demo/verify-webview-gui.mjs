...
const chrome = spawn(CHROME, ['--headless=new','--disable-gpu','--no-first-run','--user-data-dir=' + path.join(os.tmpdir(),'liuli-gui-' + process.pid),'--remote-debugging-port=' + String(PORT),'--window-size=1680,980','http://127.0.0.1:7336/?dsh-desktop-mode=compatibility&dsh-desktop-platform=win32'],{stdio:'ignore'})
...
try{
  ...
  // navigate via address bar → engine tab created
  ...
  const guiTab = (hostTabs.tabs ?? []).find(t => String(t).startsWith('browser:'))
  ...
  if (guiTab !== undefined) {
    ...
    // geometry: view should have non-zero bounds (carrier reported)
