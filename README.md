...
> 包名 `dsh-liuli-ui-enhance` · 版本 `0.1.0`
> 仓库:<https://github.com/LilycleHeart/dsh-liuli-ui-enhance.git>
...
| 📐 对话轮次刻度侧边栏 | 琉璃 时间线风格:左侧竖线刻度,胶囊沿竖线滑动,显示该轮时间/commit号/摘要,点击刻度跳转对应轮次(仅 Chat 视图显示) |
...
```
packages/client/dsh-liuli-ui-enhance/
...
│   ├── liuli-settings.ts     # 20 项设置 schema 与默认值(类型 + schemastery + 防御合并)
│   └── client/
│       ├── index.ts          # 浏览器入口:CSS 注入 + 设置分区 + 事件桥 + header slots + 悬浮球 + 预览列
│       ├── liuli.css         # 主题令牌源(亮/暗双主题 + 铬色样式 + 圆形遮罩 + 入场动画)
│       ├── liuli-css.ts      # liuli.css 的字符串化拷贝(运行时注入 <style>,幂等;含字体 @import)
│       ├── liuli-store.ts    # 设置表单 store(ui-slots EngineStore)
│       ├── liuli-palette.ts  # M3 调色板 → DSH 令牌映射(含用户气泡明暗互换)
│       ├── liuli-runtime.ts  # 设置应用运行时(isDark 竞态保护 + seq 令牌 + 壁纸承载层)
│       ├── liuli-transition.ts # 会话切换/新消息入场动画(MutationObserver 挂类 + 级联延迟)
│       ├── LiuliAppearance.tsx / .module.css   # 设置页「界面」分区
...
│       ├── TurnRail.tsx / .module.css           # 琉璃 时间线风格轮次刻度侧边栏
│       ├── FloatBall.tsx / .module.css / .types.ts  # 悬浮工具球 + 拾取模式
│       ├── PreviewPanel.tsx / .module.css       # DSH 侧边面板壳:标签条/概览/新增/右键菜单/空状态 + 宽度覆盖 + header 切换按钮
│       ├── RightSidebarPanels.tsx / .module.css  # 侧边面板标签内容:命令中心/文件树/Wiki/Git
│       ├── SidePaneIcons.tsx                     # 侧边面板图标(lucide 路径取自 参考实现 bundle,1:1)
...
│       ├── locales.ts        # liuli-appearance 文案(zh/en,键集完整性互检)
