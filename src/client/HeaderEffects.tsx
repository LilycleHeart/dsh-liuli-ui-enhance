...
/** 波形逻辑高度（px）：固定基准 —— 波形形态不随 header 拉伸变化，
 *  垂直居中（off + dh/2 = h/2，波形中心始终在 header 上下中间）。 */
const DRAW_H = 112
...
export function DenpaHeaderVoiceprint() {
  ...
  useEffect(() => {
    ...
    const drawWave = (freqData: Uint8Array, C: RGB, dark: boolean): void => {
      ...
      // 波形形态固定（DRAW_H 基准：线数/振幅/重心全基于基准，拉伸不改比例）；
      // 垂直居中：off = (h - dh)/2，波形中心 = off + dh/2 = h/2，
      // 始终在 header 上下中间。header 比基准矮时保持比例（上下微裁）。
      const dh = DRAW_H
      const off = (h - dh) / 2
      const cy = off + dh * 0.5
