 ...
 * nav[class$="_rail"] > div[class$="_marks"] > … > button[class$="_mark"]）
 * 与琉璃自绘的 TurnRail 功能重复，需整条隐藏。CSS 规则（liuli-css.ts 里
 * [class$="_slot"]:has(> nav[class$="_rail"]:has([class$="_mark"])) { display:none }）
...
/** 官方 rail 槽位：class 以 _slot 结尾，直接子 nav（class 含 _rail）内含
 *  以 _mark 结尾的元素即判定为官方 TurnNavigator（CSS Modules hash 前缀
 *  随构建变，故用后缀/包含匹配；琉璃 TurnRail 是 portal 到正文卡片的
 *  无 _slot 包装 nav，不会命中）。 */
function isNativeTurnNavigatorSlot(el: HTMLElement): boolean {
  ...
  for (const child of el.children) {
    if (child.tagName === 'NAV' && (typeof child.className === 'string') && child.className.includes('_rail')) {
      nav = child
      break
    }
  }
  if (nav === null) return false
  return nav.querySelector('[class$="_mark"]') !== null
}
