...
- **可变 split sash 直写 flexGrow 也要做像素级 clamp**：`beginSash` 的可变 split 路径曾只按 `MIN_SIZE` 比例 clamp，导致手柄能把相邻卡片拖过最小宽/高（手柄“骑到”相邻卡片上）。修复：按下时读取相邻两个 shard 的当前像素尺寸，把 `PANE_CARD_MIN_W/H`（会话列 `CONVERSATION_MIN`）换算成 `minBeforeRatio/minAfterRatio`，onMove 里 `lo = max(MIN_SIZE*sizesTotal, minBeforeRatio)`、`hi = min(sizesTotal - MIN_SIZE*sizesTotal, sizesTotal - minAfterRatio)` 后再夹取 `na`。验证：拖拽拆出标签页的 sash，两边都不会小于 240×160。
