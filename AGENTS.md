...
- **会话页头面板裁剪官方后台任务悬浮窗**：`.pane` 默认 `overflow:hidden`，header 拆成独立面板后，官方「后台任务」popover 定位在 header 内、溢出面板范围时被裁掉（元素选择器一开 popover 就消失，很难取证）。修复：在 `DockShellFrame.module.css` 给 `.pane[data-region-pane='region:conversation-header'] { overflow: visible }`，允许 popover 溢出；声纹背景层不受影响（其 `.wrap` 自身仍有 overflow:hidden 负责圆角裁剪）。验证：有后台任务运行时点开 header 里的后台任务入口，popover 完整显示不被面板截断。
...
- **win32 无边框窗口拖拽区不要放在 header 上（独立面板后会拖走窗口）**：改为 desktop 顶部 30px 透明拖动条。实现：DockShellFrame 根内渲染真实 `<div class="windowTopDrag" data-liuli-window-drag>`（不要用 `.dshDesktopFrame::before` 伪元素，Electron 对伪元素的 `-webkit-app-region` 命中不可靠），CSS `position:absolute; top:0; left:0; right:0; height:30px; z-index:200; -webkit-app-region:drag;`；`index.ts` 删除 `[data-region-pane="region:conversation-header"] header` 的 drag 规则，`SETTINGS_DEFER_CSS` 中给 `[data-liuli-window-drag]` 加 no-drag。**drag 区域无法点击穿透**：它会吞掉下方按钮的点击，必须把顶部 30px 内的交互元素 z-index 提到 201 以上让它们浮在拖拽条上方——gripCluster（z-index 201）、WindowControls 胶囊（201/202）、侧栏 logoRow（`position:relative; z-index:201`）。验证：CDP 查 `[data-liuli-window-drag]` rect 高 30、`getComputedStyle(...).webkitAppRegion === 'drag'`；顶部 30px 内的侧栏 toggle、窗口按钮、页头 gripCluster 均可点击。
